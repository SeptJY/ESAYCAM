...
@property (strong, nonatomic) UILabel *lensLabel;
...
- (UILabel *)lensLabel
{
    if (!_lensLabel) {
        ...
        _lensLabel = [self createLableText:@"Lens:" color:[UIColor whiteColor] font:setFont(14)];
    }
    return _lensLabel;
}
...
//
    ...
    CGSize lensSize = [NSString sizeWithText:@"Lens:" font:self.rateLabel.font maxSize:CGSizeMake(100, 50)];
    self.lensLabel.frame = CGRectMake(self.lineView.x + 10, self.rateLabel.y, lensSize.width, lensSize.height);
    
    self.dzNumLabel.frame = CGRectMake(self.lensLabel.x + self.lensLabel.width + 5, self.rateNumLabel.y, lableSize.width + 5, lableSize.height);
}
