...
- (UIButton *)settingBtn
{
    if (!_settingBtn) {
        ...
        [_settingBtn addTarget:self action:@selector(leftTopViewSettingBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(leftTopViewSettingBtnOnClick:)]) {
        [self.delegate leftTopViewSettingBtnOnClick:btn];
    }
}
